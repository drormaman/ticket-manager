...
function Ticket(props) {
  ...
  const body = (
    ...
    <div>
      <h5>{title}</h5>
      <span>From: <span className="userEmail">{userEmail}</span></span>
  ...
  function addExpandButton(){
    if(content.length > 250){
      return (
        <>
        <p className="ticketContent">{content.slice(0,content.indexOf(" ", 250)) + " ... "}
        <span className="expandContent" onClick={handleOpen}>see more</span></p>
        </>
      );
    }
